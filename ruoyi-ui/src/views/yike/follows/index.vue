    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['yike:follows:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['yike:follows:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listFollows, getFollows, delFollows, addFollows, updateFollows } from "@/api/yike/follows"
  ...
  methods: {
    ...
    getList() {
      ...
      listFollows(this.queryParams).then(response => {
        this.followsList = response.rows
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateFollows(this.form).then(response => {
              this.$modal.msgSuccess("修改成功")
              this.open = false
              this.getList()
            })
          } else {
            addFollows(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('yike/follows/export', {
        ...this.queryParams
