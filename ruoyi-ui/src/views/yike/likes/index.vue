...
<script>
import { listLikes, getLikes, delLikes, addLikes, updateLikes } from "@/api/yike/likes"
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除用户点赞记录编号为"' + ids + '"的数据项？').then(function() {
        return delLikes(ids)
      }).then(() => {
        this.getList()
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('yike/likes/export', {
        ...this.queryParams
